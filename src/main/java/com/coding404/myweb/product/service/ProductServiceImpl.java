...
public class ProductServiceImpl implements ProductService {
	...
	//하나의 메서드에서 여러 CRUD작업이 일어나는 경우에 한부분이 에러가 발생하면 그 에러를 처리하고, 롤백처리를 대신합니다
	@Override
	...
	public int productRegist(ProductVO vo, List<MultipartFile> list) {
		...
		for(MultipartFile file : list) {
			...
			//System.out.println( file.isEmpty() ); //비어있으면 true
			...
			try {
				...
				System.out.println("파일업로드중 error 발생");
				e.printStackTrace();
				return 0; //실패의 의미
			}
			
			
			//productUpload 테이블에 파일의 경로 인서트
